...
BEGIN
  ...
  -- 从 status_records 表获取一个现有的用户ID（如果有的话）
  SELECT DISTINCT user_id INTO v_current_user_id 
  FROM status_records 
  LIMIT 1;
  
  -- 如果没有找到，尝试从 auth.users 获取
  ...
  -- 如果还是没有，生成一个新的UUID
  IF v_current_user_id IS NULL THEN
    v_current_user_id := uuid_generate_v4();
    RAISE NOTICE '未找到现有用户，生成新的测试用户ID: %', v_current_user_id;
  ELSE
    RAISE NOTICE '使用现有用户ID: %', v_current_user_id;
  END IF;
