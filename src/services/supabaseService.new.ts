...
import {get, post, patch, rpc, handleApiError} from './postgrestApi';
import {
  User,
  Tag,
  StatusRecord,
...
class DataServiceWrapper {
   ...
   * 允许同一用户同一天多次提交（累计模式）
   */
  async submitUserStatus(statusData: Partial<StatusRecord>): Promise<StatusRecord> {
    try {
      return await dataService.submitUserStatus({
        userId: statusData.userId!,
        date: statusData.date!,
        isOvertime: statusData.isOvertime!,
        tagId: statusData.tagId,
        overtimeHours: statusData.overtimeHours,
      });
    } catch (error) {
  ...
  async getUserTagProportion(
    ...
    try {
      return await dataService.getUserTagProportion(userId, year, month);
    } catch (error) {
