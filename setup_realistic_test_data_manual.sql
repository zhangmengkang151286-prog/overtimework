-- ============================================
-- 设置真实的测试数据（手动指定用户ID版本）
-- 删除所有历史数据，使用指定用户创建今天的多次提交
-- 数据时间不超过当前时间
-- ============================================

-- 使用说明：
-- 1. 先查询你的用户ID：SELECT id, username, phone_number FROM users ORDER BY created_at DESC LIMIT 5;
-- 2. 将下面的 'YOUR_USER_ID_HERE' 替换为你的实际用户ID
-- 3. 执行整个脚本

-- 1. 删除所有历史数据并创建测试数据
DO $$
DECLARE
  v_current_user_id UUID := 'YOUR_USER_ID_HERE'::UUID; -- 👈 替换为你的用户ID
