...
DROP POLICY IF EXISTS "Allow public read access to hourly snapshots" ON hourly_snapshots;
...
-- 2. 此版本不使用 pg_cron，需要手动或通过其他方式触发快照保存
-- 3. 可以通过以下方式定期保存快照：
--    - 使用外部定时任务（如 GitHub Actions、云函数等）
--    - 在应用中定期调用 save_hourly_snapshot() 函数
-- 4. 客户端可以通过 RPC 调用 get_hourly_snapshot() 获取快照
-- 5. 手动清理旧快照：SELECT cleanup_old_hourly_snapshots();

-- ============================================
-- 如何启用 pg_cron（可选）
-- ============================================
-- 如果你的 Supabase 项目支持 pg_cron，可以执行以下命令启用：
-- 
-- 1. 启用扩展：
